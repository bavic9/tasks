...
function HeaderDropdown({ setOpenDropdown, setBoardModalOpen }) {
  ...
  return (
    ...
    >
        <div className='bg-white dark:bg-[#2b2c37] shadow-md shadow-[#364e7e1a] w-full py-6 rounded-xl'>
            <h3 className='dark:text-gray-300 text-gray-600 font-semibold mx-4 mb-8'>
                All Boards ({boards?.length})
            </h3>
            <div>
                {boards.map((board, index) => (
                    <div className={` flex items-baseline cursor-pointer dark:text-white space-x-2 px-5 py-4 ${board.isActive && 'bg-[#635fc7] rounded-r-full text-white mr-8'}`}
                    key={index} 
                    onClick={() =>{
